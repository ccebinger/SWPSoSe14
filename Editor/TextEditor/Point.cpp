...
void Point::make_Node_Changes(InternStack *changes){
    ...
    switch (type) {
    case Start:
        if(b_Cons_to_Nodes & 128)cons_to_Nodes[0]->change_Connections(128 & mask ,iString & 1 && 128 & mask,iFunctions & 1 && 128 & mask & ~iFunctions,iVariables & 1 && 128 &  mask & ~iVariables, 1, changes);
        if(b_Cons_to_Nodes & 64)cons_to_Nodes[1]->change_Connections(64 & mask ,iString & 2 && 64 & mask,iFunctions & 2 && 64 & mask & ~iFunctions,iVariables & 2 && 64 &  mask & ~iVariables, 2, changes);
        if(b_Cons_to_Nodes & 32)cons_to_Nodes[2]->change_Connections(32 & mask ,iString & 4 && 32 & mask,iFunctions & 4 && 32 & mask & ~iFunctions,iVariables & 4 && 32 &  mask & ~iVariables, 4, changes);
        if(b_Cons_to_Nodes & 16)cons_to_Nodes[3]->change_Connections(16 & mask ,iString & 8 && 16 & mask,iFunctions & 8 && 16 & mask & ~iFunctions,iVariables & 8 && 16 &  mask & ~iVariables, 8, changes);
        if(b_Cons_to_Nodes & 8)cons_to_Nodes[4]->change_Connections(8 & mask ,iString & 16 && 8 & mask,iFunctions & 16 && 8 & mask & ~iFunctions,iVariables & 16 && 8 &  mask & ~iVariables, 16, changes);
        if(b_Cons_to_Nodes & 4)cons_to_Nodes[5]->change_Connections(4 & mask ,iString & 32 && 4 & mask,iFunctions & 32 && 4 & mask & ~iFunctions,iVariables & 32 && 4 &  mask & ~iVariables, 32, changes);
        if(b_Cons_to_Nodes & 2)cons_to_Nodes[6]->change_Connections(2 & mask ,iString & 64 && 2 & mask,iFunctions & 64 && 2 & mask & ~iFunctions,iVariables & 64 && 2 &  mask & ~iVariables, 64, changes);
        if(b_Cons_to_Nodes & 1)cons_to_Nodes[7]->change_Connections(true,iString & 128 && 1 & mask,iFunctions & 128 && 1 & mask & ~iFunctions,iVariables & 128 && 1 &  mask & ~iVariables, 128, changes);
        break;
    case Lambda:
        if(b_Cons_to_Nodes & 128)cons_to_Nodes[0]->change_Connections(128 & mask & ~iString ,iString & 1 && 128 & mask,iFunctions & 1 && 128 & mask & ~iFunctions,iVariables & 1 && 128 &  mask & ~iVariables, 1, changes);
        if(b_Cons_to_Nodes & 64)cons_to_Nodes[1]->change_Connections(64 & mask & ~iString ,iString & 2 && 64 & mask,iFunctions & 2 && 64 & mask & ~iFunctions,iVariables & 2 && 64 &  mask & ~iVariables, 2, changes);
        if(b_Cons_to_Nodes & 32)cons_to_Nodes[2]->change_Connections(32 & mask & ~iString ,iString & 4 && 32 & mask,iFunctions & 4 && 32 & mask & ~iFunctions,iVariables & 4 && 32 &  mask & ~iVariables, 4, changes);
        if(b_Cons_to_Nodes & 16)cons_to_Nodes[3]->change_Connections(16 & mask & ~iString ,iString & 8 && 16 & mask,iFunctions & 8 && 16 & mask & ~iFunctions,iVariables & 8 && 16 &  mask & ~iVariables, 8, changes);
        if(b_Cons_to_Nodes & 8)cons_to_Nodes[4]->change_Connections(8 & mask & ~iString ,iString & 16 && 8 & mask,iFunctions & 16 && 8 & mask & ~iFunctions,iVariables & 16 && 8 &  mask & ~iVariables, 16, changes);
        if(b_Cons_to_Nodes & 4)cons_to_Nodes[5]->change_Connections(4 & mask & ~iString ,iString & 32 && 4 & mask,iFunctions & 32 && 4 & mask & ~iFunctions,iVariables & 32 && 4 &  mask & ~iVariables, 32, changes);
        if(b_Cons_to_Nodes & 2)cons_to_Nodes[6]->change_Connections(2 & mask & ~iString ,iString & 64 && 2 & mask,iFunctions & 64 && 2 & mask & ~iFunctions,iVariables & 64 && 2 &  mask & ~iVariables, 64, changes);
        if(b_Cons_to_Nodes & 1)cons_to_Nodes[7]->change_Connections(1 & mask & ~iString,iString & 128 && 1 & mask,iFunctions & 128 && 1 & mask & ~iFunctions,iVariables & 128 && 1 &  mask & ~iVariables, 128, changes);
        break;
    case Reflector:
        if(b_Cons_to_Nodes & 128)cons_to_Nodes[0]->change_Connections(128 & mask & ~iString ,iString & 1 && 128 & mask,iFunctions & 1 && 128 & mask & ~iFunctions,iVariables & 1 && 128 &  mask & ~iVariables, 1, changes);
        if(b_Cons_to_Nodes & 64)cons_to_Nodes[1]->change_Connections(64 & mask & ~iString ,iString & 2 && 64 & mask,iFunctions & 2 && 64 & mask & ~iFunctions,iVariables & 2 && 64 &  mask & ~iVariables, 2, changes);
    ...
    case String_Symble:
        ...
        break;
    case Funktion_Symble:
        if(b_Cons_to_Nodes & 128)cons_to_Nodes[0]->change_Connections(128 & mask ,iString & 1 && 128 & mask,128 & mask && !(1 & iStart & iFunctions),iVariables & 1 && 128 &  mask & ~iVariables, 1, changes);
        if(b_Cons_to_Nodes & 64)cons_to_Nodes[1]->change_Connections(64 & mask ,iString & 2 && 64 & mask,64 & mask && !(2 & iStart & iFunctions),iVariables & 2 && 64 &  mask & ~iVariables, 2, changes);
        if(b_Cons_to_Nodes & 32)cons_to_Nodes[2]->change_Connections(32 & mask ,iString & 4 && 32 & mask,32 & mask && !(4 & iStart & iFunctions),iVariables & 4 && 32 &  mask & ~iVariables, 4, changes);
        if(b_Cons_to_Nodes & 16)cons_to_Nodes[3]->change_Connections(16 & mask ,iString & 8 && 16 & mask,16 & mask && !(8 & iStart & iFunctions),iVariables & 8 && 16 &  mask & ~iVariables, 8, changes);
        if(b_Cons_to_Nodes & 8)cons_to_Nodes[4]->change_Connections(8 & mask ,iString & 16 && 8 & mask,8 & mask && !(16 & iStart & iFunctions),iVariables & 16 && 8 &  mask & ~iVariables, 16, changes);
        if(b_Cons_to_Nodes & 4)cons_to_Nodes[5]->change_Connections(4 & mask ,iString & 32 && 4 & mask,4 & mask && !(32 & iStart & iFunctions),iVariables & 32 && 4 &  mask & ~iVariables, 32, changes);
        if(b_Cons_to_Nodes & 2)cons_to_Nodes[6]->change_Connections(2 & mask ,iString & 64 && 2 & mask,2 & mask && !(64 & iStart & iFunctions),iVariables & 64 && 2 &  mask & ~iVariables, 64, changes);
        if(b_Cons_to_Nodes & 1)cons_to_Nodes[7]->change_Connections(1 & mask ,iString & 128 && 1 & mask,1 & mask && !(128 & iStart & iFunctions),iVariables & 128 && 1 &  mask & ~iVariables, 128, changes);
        break;
    case Variable_Symbol:
        if(b_Cons_to_Nodes & 128)cons_to_Nodes[0]->change_Connections(128 & mask ,iString & 1 && 128 & mask,iFunctions & 1 && 128 & mask & ~iFunctions,128 & mask && !(1 & iStart & iVariables), 1, changes);
        if(b_Cons_to_Nodes & 64)cons_to_Nodes[1]->change_Connections(64 & mask ,iString & 2 && 64 & mask,iFunctions & 2 && 64 & mask & ~iFunctions,64 & mask && !(2 & iStart & iVariables), 2, changes);
        if(b_Cons_to_Nodes & 32)cons_to_Nodes[2]->change_Connections(32 & mask ,iString & 4 && 32 & mask,iFunctions & 4 && 32 & mask & ~iFunctions,32 & mask && !(4 & iStart & iVariables), 4, changes);
        if(b_Cons_to_Nodes & 16)cons_to_Nodes[3]->change_Connections(16 & mask ,iString & 8 && 16 & mask,iFunctions & 8 && 16 & mask & ~iFunctions,16 & mask && !(8 & iStart & iVariables), 8, changes);
        if(b_Cons_to_Nodes & 8)cons_to_Nodes[4]->change_Connections(8 & mask ,iString & 16 && 8 & mask,iFunctions & 16 && 8 & mask & ~iFunctions,8 & mask && !(16 & iStart & iVariables), 16, changes);
        if(b_Cons_to_Nodes & 4)cons_to_Nodes[5]->change_Connections(4 & mask ,iString & 32 && 4 & mask,iFunctions & 32 && 4 & mask & ~iFunctions,4 & mask && !(32 & iStart & iVariables), 32, changes);
        if(b_Cons_to_Nodes & 2)cons_to_Nodes[6]->change_Connections(2 & mask ,iString & 64 && 2 & mask,iFunctions & 64 && 2 & mask & ~iFunctions,2 & mask && !(64 & iStart & iVariables), 64, changes);
        if(b_Cons_to_Nodes & 1)cons_to_Nodes[7]->change_Connections(1 & mask ,iString & 128 && 1 & mask,iFunctions & 128 && 1 & mask & ~iFunctions,1 & mask && !(128 & iStart & iVariables), 128, changes);
        break;
    default:
        if(b_Cons_to_Nodes & 128)cons_to_Nodes[0]->change_Connections(128 & mask ,iString & 1 && 128 & mask,iFunctions & 1 && 128 & mask & ~iFunctions,iVariables & 1 && 128 &  mask & ~iVariables, 1, changes);
        if(b_Cons_to_Nodes & 64)cons_to_Nodes[1]->change_Connections(64 & mask ,iString & 2 && 64 & mask,iFunctions & 2 && 64 & mask & ~iFunctions,iVariables & 2 && 64 &  mask & ~iVariables, 2, changes);
        ...
        if(b_Cons_to_Nodes & 4)cons_to_Nodes[5]->change_Connections(4 & mask ,iString & 32 && 4 & mask,iFunctions & 32 && 4 & mask & ~iFunctions,iVariables & 32 && 4 &  mask & ~iVariables, 32, changes);
        if(b_Cons_to_Nodes & 2)cons_to_Nodes[6]->change_Connections(2 & mask ,iString & 64 && 2 & mask,iFunctions & 64 && 2 & mask & ~iFunctions,iVariables & 64 && 2 &  mask & ~iVariables, 64, changes);
        if(b_Cons_to_Nodes & 1)cons_to_Nodes[7]->change_Connections(1 & mask ,iString & 128 && 1 & mask,iFunctions & 128 && 1 & mask & ~iFunctions,iVariables & 128 && 1 &  mask & ~iVariables, 128, changes);
        break;
    }
}
